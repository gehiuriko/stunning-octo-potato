...
async function headObject(env,key){
  // Backblaze B2 can return 403 for S3 HEAD with some restricted app-key /
  // compatibility combinations even though GET on the same object is allowed.
  // Probe only one byte with GET instead. This verifies the object exists
  // without downloading the ZIP.
  const res=await b2Fetch(env,key,{method:"GET",headers:{"Range":"bytes=0-0"}});
  ...
  throw new Error(`B2 object check ${res.status}${detail?`: ${detail}`:""}`);
}
